...
const dirRE = /^v-|^@|^:/;
const vuejsi18n = require('./vuejsi18n3');
    ...
    .map((node, i) => {
          ...
          tokens.map((token, i) => {
            ...
            if (token.type === 'static') {
              ...
              if (isEmpty(token.binding) || !hasChinese.test(token.binding)) {
                ...
                tokenTransformed = `{{$t('${key}.${decodeOriginString(
                  token.binding
                )}')}}`;
            ...
            transformedString += `${originString.slice(
              preToken.end,
              token.start
            )}${tokenTransformed}`;
      ...
      if (node.type === 'attr') {
        ...
        if (hasChinese.test(value)) {
          transformedString = `:${originString.replace(
            value,
            `$t('${key}.${decodeOriginString(value)}')`
          )}`;
