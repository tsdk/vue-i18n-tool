    ...
    <div class="common-content">
      <div class="pd20">
        ...
        <el-tooltip
          class="box-item"
          effect="dark"
          content="记录用户反馈的内容打标错误，及 提报/修正 等操作"
          placement="top-start"
        >
      ...
      <div class="flex-row" style="padding: 0 20px 20px 20px">
        <div class="flex-row" style="flex-wrap: wrap; gap: 15px">
          ...
          <el-input class="w-200 w-search" placeholder="内容标题" v-model="instance.queryParams.text" clearable>
            <template #suffix>
              <svg-icon name="smc-search" className="pointer" />
            </template>
          </el-input>
          <el-input class="w-200 w-search" placeholder="内容ID" v-model="instance.queryParams.itemId" clearable>
            <template #suffix>
              <svg-icon name="smc-search" className="pointer" />
            </template>
          </el-input>
      ...
      <div class="table">
        ...
        >
          ...
          <el-table-column label="内容信息" property="title" min-width="440" show-overflow-tooltip>
            <template #default="scope">
              <div v-if="scope.row.itemId" class="desc" style="font-size: 13px">内容ID:{{ scope.row.itemId }}</div>
            ...
            <template #default="scope">
              <div v-if="scope.row.status === 'wait_fix'">
                <el-button
                  ...
                  v-permission="{
                    permissionKey: 'OPINION_CONTENT_TAG_OPERATE_FIX'
                  }"
                >
                  立即修正
                </el-button>
